...
function polyIntersection(poly1, poly2) {
  for (var i = 0; i < poly1.length; i++) {
    for (var j = 0; j < poly2.length; j++) {
      var touch = getIntersection(poly1[i], poly1[(i + 1) % poly1.length], //taking one point and the next point in the first polygon and connect them together
      ...
      poly2[j], poly2[(j + 1) % poly2.length]); // console.log(touch);
